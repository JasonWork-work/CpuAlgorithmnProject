...
public void runFCFS() throws InterruptedException {
   cpuItems.sort(Comparator.comparingInt(m -> m.arrivalTime));
    ...
    for (Metrics m : cpuItems) {
        ...
        for (int i = 0; i < m.burstTime; i++) {
            appendToGanttChart("P" + (m.index + 1));
            Thread.sleep(100);
            currentTime++;
        }
 ...
 public void runSJF() throws InterruptedException {
   List<Metrics> processes = new ArrayList<>(cpuItems);
    ...
    while (completed.size() < cpuItems.size()) {
        ...
        // If none are ready, advance time
        ...
        Metrics m = readyQueue.get(0); // shortest burst time

        if (m.startTime == -1) {
    m.startTime = currentTime; // only first time!
}
 ...
 public void runSRTF() throws InterruptedException {
    ...
    while (completed < processes.size()) {
        ...
        appendToGanttChart("P" + (current.index + 1));
        currentTime++;
...
while (completed < processes.size()) {
    for (Metrics m : processes) {
        if (m.arrivalTime <= currentTime && m.remainingTime > 0 && !queues.get(m.queueLevel).contains(m)) {
            queues.get(m.queueLevel).add(m);
        }
    }
    ...
    if (current.remainingTime == 0) {
        ...
        current.waitingTime = current.turnaroundTime - current.burstTime;
        completed++;
    } else if (usedAllotment.get(current) >= allotment && level + 1 < numQueues) {
        current.queueLevel = level + 1;
        usedAllotment.put(current, 0);
        queues.get(current.queueLevel).add(current);
    } else {
        queues.get(current.queueLevel).add(current);
    }
}
